...
export class SvgComponent implements OnInit {
  ...
  CalculateCircle(num:number[],circleR:number){
    ...
    for(let n in num){
        ...
        console.log(alldeg);
        if(alldeg<90){
          let x = circleR + circleR/Math.cos(alldeg*2*Math.PI/360)
          this.loc.push(x,0);
        }
        else if(alldeg==90){
          ...
          let y = circleR + circleR/Math.cos((alldeg-90)*2*Math.PI/360)
          ...
          let x = circleR - circleR/Math.cos((alldeg-180)*2*Math.PI/360)
          ...
          let y = circleR - circleR/Math.cos((alldeg-270)*2*Math.PI/360)
          this.loc.push(0,y);
        }
    }
  }
